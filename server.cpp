...
void Server::_botCmd(Client *client)
{
    ...
    {
        sendMsg(client->getFd(), ":" + client->get_nick_adresse() + " NOTICE " + client->getNick() + " :" + "BOT :Welcome to localhost Bot ! Please insert bot Quote or bot Joke or bot Time and enjoy!\r\n");
        return ;
    }
    ...
    else if (client->commande_splited[1] == "Quote")
    {
        ...
        file.close();
        sendMsg(client->getFd(), ":" + client->get_nick_adresse() + " NOTICE " + client->getNick() + " :" + " ,Here is your Quote for today: " + quote + "\r\n");
    }
    else if (client->commande_splited[1] == "Joke")
    {
        ...
        for (int i = 0; i < line; i++)
            std::getline(file, quote);
        file.close();
          sendMsg(client->getFd(), ":" + client->get_nick_adresse() + " NOTICE " + client->getNick() + " :" + " ,Here is your Joke for today: " + quote + "\r\n");
    }
    else if (client->commande_splited[1] == "Time")
    {
        ...
        std::string time = oss.str();
          sendMsg(client->getFd(), ":" + client->get_nick_adresse() + " NOTICE " + client->getNick() + " :" + " Here is the time: " + time + "\r\n");

    }
    else
    {
          sendMsg(client->getFd(), ":" + client->get_nick_adresse() + " NOTICE " + client->getNick() + " :" + " BOT :Wrong command!\r\n");
        return ;
    }
        ...
        if (len == 0)
        {
            for (size_t i = 0; i < _channels.size(); i++)
            {
                if (_channels[i]->isMember(client))
                {
                    ...
                    _channels[i]->sendToMembers(":" + client->get_nick_adresse() + " QUIT : Client closed connection");
                }
            }
            close(i->fd);
            clients.erase(i->fd);
        ...
        for (unsigned long i = 0; i < poll_vec.size(); i++)
        {
            ...
            if (current.fd == this->fd)
            {
               try
               {
                    ...
                    std::cout << "Client was pushed to vector" << std::endl;
               }
...
void Server::_execute_commands(Client *client) 
{
    if (client->commande_splited[0] == "JOIN" || client->commande_splited[0] == "join")
        ...
        sendMsg(client->getFd(), ":" + client->get_nick_adresse() + ":localhost 421 " + client->getNick() + " " + client->commande_splited[0]  + " :Unknown command\r\n");
}
...
void Server::_wallopsCmd(Client *client)
{
    ...
    if(client->commande_splited.size() < 2)
    {
        std::string errorMessage = ":" + client->get_nick_adresse() + "  412  " + (client->getNick().empty() ? "*" : client->getNick()) + " " + ":No text to send\r\n";
...
void Server::find_channel_and_sendmsg1(Client *client, std::string &target, std::string &message, bool error)
{
    ...
    if (!tmp && error)
    {
        sendMsg(client->getFd(), ":" + client->get_nick_adresse() + " 403 " + (client->getNick().empty() ? "*" : client->getNick()) + " " + target + " :No such channel\r\n");
        return ;
    }
    else if (!tmp && !error)
        return ;
    else
    {
        ...
        tmp->sendToOthers(":" + client->get_nick_adresse() + " " + flag_com + " " + tmp->getChannelName() + " :" + message + "\r\n", client->getFd());
    }
    return ;
}
...
void Server::find_client_and_sendmsg1(Client *client, std::string &target, std::string &message, bool error)
{
    ...
    if (tmp != NULL)
    {
        sendMsg(tmp->getFd(), ":" + tmp->get_nick_adresse() + " " + flag_com + " " + tmp->getNick() + " :" + message + "\r\n");
        return ;
...
void Server::_privMsgCmd(Client *client, bool error)
{
    ...
    if (client->commande_splited.size() < 2)
    {
        sendMsg(client->getFd(), ":" + client->get_nick_adresse() + " 411 " + (client->getNick().empty() ? "*" : client->getNick()) + " " + " :No recipient given " + flag_com + " " + "\r\n");
        return;
    }
    else if (client->commande_splited.size() == 2)
    {
        sendMsg(client->getFd(), ":" + client->get_nick_adresse() + " 412 " + (client->getNick().empty() ? "*" : client->getNick()) + " " + " :No text to send " + flag_com + " " + "\r\n");
        return;
    }
...
void Server::_joinCmd(Client *client)
{
    ...
    for (int i = 0; i < size; i++)
    {
        if (channels[i][0] == '#' || channels[i][0] == '&')
        {
            ...
            sendMsg(client->getFd(), ":localhost 403 " + client->getNick() + " " + channels[i] + " :No such channel\r\n");
        }
    }
}
...
void Server::_modeCmd(Client *client)
{
    if (client->commande_splited.size() < 3)
    {
         sendMsg(client->getFd(), ":localhost 461 " + client->getNick() + " MODE :Not enough parameters\r\n");
        return ;
    }
    ...
    if (!channel)
    {
        sendMsg(client->getFd(), ":localhost 403 " + client->getNick() + " " + client->commande_splited[1] + " :No such channel\r\n");
        return ;
    }
    ...
    else if (mode == "+o" || mode == "-o")
    {
        ...
        if (!Newclient)
            sendMsg(client->getFd(), ":localhost 401 " + client->commande_splited[1] + " " + client->getNick() + " :No such nick/channel\r\n");
        else
        ...
        sendMsg(client->getFd(), ":localhost 472 " + client->getNick() + " " + mode.substr(1, 2) + " :is unknown mode char to me\r\n");
}
...
void Server::_topicCmd(Client *client) 
{
    ...
    if (client->commande_splited.size() < 2)
    {
        sendMsg(client->getFd(), ":localhost 461 " + client->getNick() + " TOPIC :Not enough parameters\r\n");
        return ;
    }
...
void Server::_inviteCmd(Client *client)
{
    if (client->commande_splited.size() < 3)
    {
        sendMsg(client->getFd(), ":localhost 461 " + client->getNick() + " INVITE :Not enough parameters\r\n");
        return ;
    }
    Client *Newclient = findClientByNick(client->commande_splited[1]);
    if (!Newclient)
    {
        sendMsg(client->getFd(), ":localhost 401 " + client->commande_splited[2] + " " + client->getNick() + " :No such nick/channel\r\n");
        return ;
    }
    Channel *channel = _findChannel(client->commande_splited[2]);
    if (!channel)
    {
        sendMsg(client->getFd(), ":localhost 403 " + client->getNick() + " " + client->commande_splited[2] + " :No such channel\r\n");
        return ;
    }
...
void Server::_partCmd(Client *client)
{
    if (client->commande_splited.size() < 2)
    {
        sendMsg(client->getFd(), ":localhost 461 " + client->getNick() + " PART :Not enough parameters\r\n");
        return ;
    }
...
void Server::_kickCmd(Client *client)
{
    ...
    {
        sendMsg(client->getFd(), ":localhost 461 " + client->getNick() + " KICK :Not enough parameters\r\n");
        return ;
    }
    Channel *channel = _findChannel(client->commande_splited[1]);
    if (!channel)
...
void Server::_operCmd(Client *client)
{
    ...
    {
        sendMsg(client->getFd(), ":localhost 464 " + client->getNick() + " :Password incorrect\r\n");
        return ;
    }
    if (client->commande_splited[1] == client->getNick())
    {
        client->setOperatorStatus(true);
        sendMsg(client->getFd(), ":localhost 381 " + client->getNick() + " :You are now an localhost operator\r\n");
        sendMsg(client->getFd(), ":" + client->get_nick_adresse() + " " + "MODE +O\r\n");
    }
}
